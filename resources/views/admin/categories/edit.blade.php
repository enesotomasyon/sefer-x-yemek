...
@section('content')
<div class="max-w-2xl">
    <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="card p-8">
        @csrf
        @method('PUT')
        <div class="mb-6">
        ...
        <div class="mb-6">
            ...
            @if($category->image)
            <div class="mb-3">
                <img src="{{ asset('storage/' . $category->image) }}" class="w-32 h-32 rounded-lg object-cover border-2 border-gray-200">
            </div>
            @endif
